...
m = Module("02_dsp", "03_fourier_from_the_ground_up")

m.md(r'''# Lesson SIG 3: The Fourier Transform from the Ground Up {{VARIANT}}
...
'''def dft(x: np.ndarray) -> np.ndarray:
    ...
    """
    # TODO: N = len(x); build the index vectors n and k
    # TODO: form the matrix exp(-2j * pi * k[:, None] * n[None, :] / N)
    # TODO: return that matrix applied to x
    raise NotImplementedError("Implement dft")''',
'''def dft(x: np.ndarray) -> np.ndarray:
    """Discrete Fourier transform, computed directly from the definition.
...
'''def leakage_floor_db(x: np.ndarray, window: np.ndarray | None = None) -> float:
    ...
    """
    # TODO: multiply x by the window when one is given
    # TODO: take np.fft.rfft and its magnitude
    # TODO: find the peak bin, mask out that bin and 5 bins either side
    # TODO: return 20 * log10(max of what remains / peak)
    raise NotImplementedError("Implement leakage_floor_db")''',
'''def leakage_floor_db(x: np.ndarray, window: np.ndarray | None = None) -> float:
    """Worst sidelobe level, in dB relative to the peak, away from the peak bin.
...
m.emit()
verify("02_dsp", "03_fourier_from_the_ground_up")
print("  SIG 3 OK")
